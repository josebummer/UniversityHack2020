...
def main():
    ...
    class_weights_factor = {'RESIDENTIAL': 4.812552140340716e-06*train.shape[0],
    ...
    for label in progressbar.progressbar(np.unique(labels_ini)):
        ...
        f1w_factor = get_weight_f1(class_weight_factor)
        f1w_scorer_factor = make_scorer(f1w_factor)
        ...
        sample_weights_tst_factor = np.array([class_weight_factor[i] for i in y_test])
        ...
        for idx in progressbar.progressbar(range(len(grids))):
            ...
            print('Weighted clasification factor:')
            print(classification_report(y_test, y_pred, sample_weight=sample_weights_tst_factor))
